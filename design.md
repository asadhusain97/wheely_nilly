...
Ticker cards carry the hierarchy and begin collapsed until the user opens them. Their header identifies the symbol and actual share coverage; the surrounding section already communicates whether a ticker is manually tracked. Do not add a redundant “Playbook” badge. A ticker has one goal across every enabled strategy. Each strategy shows its effective-rule summary, scan action, and results. Candidate details may expand within the card. Net contract credit and period return remain more prominent than annualized return.
