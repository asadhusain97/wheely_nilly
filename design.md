...
| Protect blue | `#376f9e` | Protect goal identity |
| Income green | `#23704a` | Income goal identity |
| Exit red | `#c33c50` | Exit goal identity |
| Acquire violet | `#6c5bb7` | Acquire goal identity |
...
Adding a ticker opens a glass sheet. The flow asks for a provider-verified instrument, strategy (CSP by default), and goal. It saves through the existing strategy-settings document and does not expose advanced rule controls. Instrument results show the symbol, recognizable name, type, and an exchange when the provider supplies one. Never present a country or region as an exchange.

Goal identity is consistent wherever a goal appears: Protect is blue, Income is green, Exit is red, and Acquire is violet. Goal choices use segmented tabs rather than native dropdowns in focused creation flows.

Ticker cards carry the hierarchy. Their header identifies the symbol and actual share coverage; the surrounding section already communicates whether a ticker is manually tracked. Do not add a redundant “Playbook” badge. Each enabled strategy shows its goal, effective-rule summary, scan action, and results. Candidate details may expand within the card. Net contract credit and period return remain more prominent than annualized return.
...
After a scan, show only the quote time and compact source attribution below it. An Alpha result reads “Alpha Vantage options · Yahoo Finance stock”; a Yahoo option fallback reads “Yahoo Finance.” Do not display fallback, unofficial, degraded, cache, or warning prose in this compact line. When no contracts match, explain that the available contracts missed the playbook and name up to three main filters without showing exclusion counts.

Brief confirmations appear above the bottom navigation as pearl-glass notices. Success uses a small Income green mark; errors use a small Exit red mark. Never use a solid black toast or a full colored panel for routine confirmation.
