...
Settings displays `closeAtProfitCapture` as `Close when premium captured` and edits it as a percentage. The lower Keep Shares threshold removes assignment exposure sooner after much of the credit is earned. The high Exit and Entry thresholds avoid having the Close signal routinely fight an assignment-oriented goal.
